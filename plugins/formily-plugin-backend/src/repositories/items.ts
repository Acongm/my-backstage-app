...
export async function list(knex: any) {
  return await knex('formily_items').select('*').orderBy('created_at', 'desc');
}

export async function getById(knex: any, id: number) {
  return await knex('formily_items').where({ id }).first();
}
...
export async function create(
  ...
  await knex('formily_items').insert({ 
    name: input.name, 
    type: input.type, 
    description: input.description,
    metadata: input.metadata ? JSON.stringify(input.metadata) : null
  });
  return await knex('formily_items').orderBy('id', 'desc').first();
}
...
export async function update(
  ...
  const updated = await knex('formily_items').where({ id }).update(patch);
  if (!updated) return undefined;
  return await knex('formily_items').where({ id }).first();
}

export async function deleteById(knex: any, id: number) {
  return await knex('formily_items').where({ id }).delete();
}
