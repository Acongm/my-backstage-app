...
export class TemplateClient {
   ...
   * 获取所有项目
   */
  async listItems(): Promise<TemplateItem[]> {
    const baseUrl = await this.discoveryApi.getBaseUrl('template');
    ...
    return (body.items ?? []) as TemplateItem[];
  }

  /**
   * 根据 ID 获取单个项目
   */
  async getItem(id: number): Promise<TemplateItem> {
    const baseUrl = await this.discoveryApi.getBaseUrl('template');
   ...
   * 创建新项目
   */
  async createItem(input: CreateItemInput): Promise<TemplateItem> {
    const baseUrl = await this.discoveryApi.getBaseUrl('template');
    const res = await this.fetchApi.fetch(`${baseUrl}/api`, {
      method: 'POST',
      headers: {
   ...
   * 更新项目
   */
  async updateItem(id: number, changes: UpdateItemInput): Promise<TemplateItem> {
    const baseUrl = await this.discoveryApi.getBaseUrl('template');
    const res = await this.fetchApi.fetch(`${baseUrl}/api/${id}`, {
      method: 'PUT',
      headers: {
